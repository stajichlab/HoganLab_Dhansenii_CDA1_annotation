...
do
  ...
  if [ ! -s $FOLDER/${SRA}_1.fastq.gz ]; then
	time parallel-fastq-dump -T $SCRATCH -O $FOLDER --threads $CPU --split-files --gzip --sra-id $SRA
	#time fasterq-dump --curcache 1G --bufsize 500M -t $SCRATCH -m $MEM -O $FOLDER/$SPECIES  --threads $CPU --split-files $SRA
  fi
done
