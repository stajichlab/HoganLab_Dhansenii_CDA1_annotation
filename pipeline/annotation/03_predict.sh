...
INDIR=genomes
OUTDIR=input_clean_genomes
SAMPLES=samples.csv
BUSCOSEED=debaryomyces_hansenii
...
do
    ...
    funannotate predict --keep_no_stops --SeqCenter WVU -i $GENOME \
		...
		--cpus $CPU --min_training_models 50 --max_intronlen 1200 --name $LOCUS \
		--min_protlen 30 --tmpdir $SCRATCH  --busco_db ${BUSCO_LINEAGE}_odb10 --table $TRANSL_TABLE
done
